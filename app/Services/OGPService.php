...
class OGPService implements \Iterator
{
  /**
   ...
   */
    static private function _parse($HTML, $URI) {
        ...
        if (!isset($page->values['image'])) {
            ...
            if ($imgs->length > 0 ) {
                $domattr = $imgs->item(( $imgs->length < 2 ? 0 : 1) )->attributes->getNamedItem('src');
                if ($domattr) {

                  if (0 === strpos($domattr->value, "http")) {
                    $imageUrl = $domattr->value;
                  }else if (0 === strpos($domattr->value, "/")){
                    $imageUrl = parse_url($URI, PHP_URL_SCHEME)."://".parse_url($URI, PHP_URL_HOST). $domattr->value;
                  }else{
                    $imageUrl = $URI . $domattr->value;
                  }

                  $page->_values['image'] = $imageUrl;
                  $page->_values['image_src'] = $imageUrl;
                }
            }
        }
        return $page;
